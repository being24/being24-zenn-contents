...
本記事では、$\LaTeX$自体の使い方については解説しません。
$\LaTeX$の使い方については[こちらのサイト](https://www.learnlatex.org/ja/)が入門について詳しく扱ってくれています（pLaTeXですが）。
よって、こちらを参照してください。
...
研究室内で使って見た結果、いくつかの問題が発生したのでそれについて追記します。
...
.latexmkrcが用意されているので、コンパイルボタンを押すと.latexmkrcに従ってコンパイルが行われます。

Overleafの場合は、新規プロジェクトをクリックし、プロジェクトのアップロードからzipファイルをアップロードしてください。（プロジェクト名は適したものに変更してください）

その後、主要文章がmain.texであることを確認した上で、コンパイルを行ってください。
upLaTeXを使用する場合は、メニューのコンパイラをLaTeXに、LuaLaTeXを使用する場合は、メニューのコンパイラをLuaLaTeXに変更してください。
...
2024/10/19追記：jlreqの更新により、この問題は解消されました。
